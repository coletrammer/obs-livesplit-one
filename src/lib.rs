...
fn log(level: Level, target: &str, args: &fmt::Arguments) {
...
unsafe fn parse_settings(settings: *mut obs_data_t) -> Settings {
    unsafe {
        ...
        let local_auto_splitter = {
            let uses_local_auto_splitter =
                obs_data_get_bool(settings, SETTINGS_LOCAL_AUTO_SPLITTER);
        ...
        let splits_path =
            CStr::from_ptr(obs_data_get_string(settings, SETTINGS_SPLITS_PATH).cast());
    ...
    unsafe fn render(&mut self) {
        unsafe {
            ...
            #[cfg(feature = "auto-splitting")]
            {
                ...
                if let Some(auto_splitter_widgets) =
                    self.global_timer.auto_splitter.settings_widgets()
                {
...
unsafe extern "C" fn media_get_time(data: *mut c_void) -> i64 {
    unsafe {
        ...
        let time =
            timer.snapshot().current_time()[timer.current_timing_method()].unwrap_or_default();
                ...
                if let (Ok(setting_key), Ok(widget_description)) = (setting_key, widget_description)
                {
                    match &widget.kind {
                        ...
                        WidgetKind::Choice {
                            ...
                            if let Ok(default_option_key) = CString::from_vec_with_nul(
                                format!("{}\0", default_option_key).into(),
                            ) {
    ...
    unsafe {
        ...
        #[cfg(feature = "auto-splitting")]
        {
            ...
            loop {
                ...
                for widget in state.auto_splitter_widgets.iter() {
                    let key = &widget.key;
                    let Ok(data_key) = CString::new(format!("auto_splitter_setting_{}", key))
                    else {
                        continue;
                    };
